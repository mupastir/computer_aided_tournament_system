...
from participant.services.get_ratings import (
...
class RatingActionsTestCase(TestCase):
    ...
    def setUpTestData(cls):
        cls.user = User.objects.create_user(
            username="test", email="test@test.com", password="top_secure", gender="m"
        )
    ...
    def test_add_rating_to_player(self):
        ...
        assert (
            self.player.rating.filter(type=CompetitionTypeChoices.BEACH_VOLLEY.value)[
                0
            ].points
            == 0
        )
        assert (
            self.player.rating.filter(type=CompetitionTypeChoices.PARK_VOLLEY.value)[
                0
            ].points
            == 0
        )
    ...
    def test_getting_rating_for_player(self):
        add_rating_to_player(self.player)
        assert (
            get_rating_for_player(self.player, CompetitionTypeChoices.PARK_VOLLEY.value)
            == 0
        )
        assert (
            get_rating_for_player(
                self.player, CompetitionTypeChoices.BEACH_VOLLEY.value
            )
            == 0
        )
