...
def add_player(context):
    context.response = api_post("/api/participant/players/", context.user)


@when("User add referee role")
def add_referee(context):
    context.response = api_post("/api/participant/referees/", context.user)


@when("User create {team_title}")
def add_team(context, team_title):
    context.response = api_post(
        "/api/participant/teams/", context.user, {"title": team_title}
    )
...
def add_players_to_team(context, team_title):
    ...
    context.response = api_put(
        f"/api/participant/teams/{team.id}/player/{player.id}/", context.user
    )
