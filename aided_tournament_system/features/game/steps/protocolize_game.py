...
def referee_scorer(context, username, role):
    context.user = User.objects.create_user(
        username=username, email=username + "@gmail.com", password="top_secure"
    )
    Referee.objects.create(user_id=context.user.id, role=role)


@given("Player")
def create_player(context):
    context.user = User.objects.create_user(
        username="test", email="test@gmail.com", password="top_secure"
    )
