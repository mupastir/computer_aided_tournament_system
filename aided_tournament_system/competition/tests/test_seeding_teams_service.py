...
class SeedingTeamsTestCase(TestCase):
    def test_seeding_teams(self):
        ...
        schedule_creator = ScheduleCreator(competition.schedule_system, competition.id)
        schedule_creator.create_schedule()
        for team in Team.objects.all():
            Application.objects.create(competition=competition, team=team)
        seeding_teams_service = SeedingTeamsService(competition.id)
        seeding_teams_service.seed()

        assert (
            Game.objects.get(competition=competition, game_number=1).home_team_id
            == Team.objects.get(title="carrot").id
        )
        assert (
            Game.objects.get(competition=competition, game_number=1).away_team_id
            is None
        )

        assert (
            Game.objects.get(competition=competition, game_number=2).home_team_id
            == Team.objects.get(title="cristal").id
        )
        assert (
            Game.objects.get(competition=competition, game_number=2).away_team_id
            == Team.objects.get(title="beluga").id
        )

        assert (
            Game.objects.get(competition=competition, game_number=3).home_team_id
            == Team.objects.get(title="indigo").id
        )
        assert (
            Game.objects.get(competition=competition, game_number=3).away_team_id
            == Team.objects.get(title="cookie").id
        )

        assert (
            Game.objects.get(competition=competition, game_number=4).home_team_id
            == Team.objects.get(title="malvin").id
        )
        assert (
            Game.objects.get(competition=competition, game_number=4).away_team_id
