...
class Ranking(UUIDTimeStampModel):
    ...
    competition = models.ForeignKey(
        "Competition", on_delete=models.CASCADE, verbose_name="competition"
    )
    ranking = models.IntegerField(verbose_name="ranking points")
    objects = RankingsManager()

    def __str__(self):
        return f"place {self.place}. " f"comp: {self.competition.title}"
...
class Competition(UUIDTimeStampModel):
    title = models.CharField(max_length=300, verbose_name="title", unique=True)
    ...
    schedule_system = models.CharField(
        ...
        verbose_name="Schedule by number " "of teams participated",
    )
